  ...
  const fetchParticipants = async () => {
    try {
      ...
      if (response.data.success) {
        const normalizedData = response.data.data.map((item: Participant) => {
          const modifiedDate = item.date.replace(/(\.\d{2})(\d)/, "$1$2");
    ...
    <div className="p-4 mx-auto max-w-7xl">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-300">
          <thead>
            <tr>
              <th className="px-4 py-2 font-medium text-left bg-gray-100 border">
                번호
              </th>
              <th className="px-4 py-2 font-medium text-left bg-gray-100 border">
                이름
          ...
          <tbody>
            {participants.map((participant, index) => (
              <tr key={participant.id} className="hover:bg-gray-50">
                ...
                {participant.quiz && (
                  <td className="px-4 py-2 border">
                    {participant.quiz.answer}
                  </td>
                )}
                {participant.snack && (
                  <td className="px-4 py-2 border">
                    {participant.snack.phone}
                  </td>
                )}
              </tr>
            ))}
