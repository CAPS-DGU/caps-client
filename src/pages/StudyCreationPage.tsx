...
const StudyCreationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post<StudyData>("/api/study", formData, {
        headers: {
      ...
      if (axios.isAxiosError(err) && err.response) {
        alert(err.response.data.message || "스터디 생성에 실패했습니다.");
      } else {
  ...
  return (
    <div className="flex justify-center p-4">
      <div className="w-full max-w-2xl">
        <h1 className="mb-6 text-2xl font-bold text-center">스터디 생성</h1>
