...
Node.js 24와 npm을 사용합니다. `npm ci`를 실행하면 Husky 커밋 훅이 설치됩니다.
커밋할 때 lint-staged가 스테이징한 파일만 Prettier로 정리합니다.

- `npm run format`: 수작업으로 관리하는 소스와 설정 전체 포맷팅
- `npm run format:check`: 포맷 검사
- `npm test`: 위키 렌더링·미리보기·각주 및 메뉴 회귀 테스트
- `npm run build`: 프로덕션 빌드
- `npx playwright install chromium` 후 `npm run test:e2e`: 로컬 빌드에서 PC·모바일 브라우저 테스트(API 응답은 테스트 데이터 사용)

자동 생성한 API 코드, OpenAPI 명세, 잠금 파일은 포맷팅에서 제외합니다.
PR에서는 포맷 검사, 테스트, 빌드를 실행합니다.
